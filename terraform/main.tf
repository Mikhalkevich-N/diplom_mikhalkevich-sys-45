...
resource "yandex_vpc_network" "network-dip" {
  name        = "network-dip"
  description = "Network dip"
}

resource "yandex_vpc_subnet" "a-subnet-dip" {
  name           = "a-subnet-dip"
  v4_cidr_blocks = ["192.168.10.0/24"]
  zone           = "ru-central1-a"
  network_id     = yandex_vpc_network.network-dip.id
  route_table_id = yandex_vpc_route_table.a-b-subnet-route-table.id
}

resource "yandex_vpc_subnet" "b-subnet-dip" {
  name           = "b-subnet-dip"
...
resource "yandex_vpc_subnet" "d-subnet-dip" {
  name           = "d-subnet-dip"
...
resource "yandex_compute_instance" "bastion" {
  name  = "bastion"
  hostname = "bastion"
  zone  = yandex_vpc_subnet.d-subnet-dip.zone
  ...
  network_interface {
    subnet_id = "${yandex_vpc_subnet.d-subnet-dip.id}"
    nat = true
    ipv4 = true
...
resource "yandex_compute_instance" "nginx-web-1" {
  ...
  zone  = yandex_vpc_subnet.a-subnet-dip.zone
  platform_id     = "standard-v3"
  resources {
    cores         = 2
    core_fraction = 20
  ...
  network_interface {
    subnet_id = "${yandex_vpc_subnet.a-subnet-dip.id}"
    ipv4 = true
...
resource "yandex_compute_instance" "nginx-web-2" {
  ...
  zone  = yandex_vpc_subnet.b-subnet-dip.zone
  platform_id     = "standard-v3"
  resources {
    cores         = 2
    core_fraction = 20
  ...
  network_interface {
    subnet_id = "${yandex_vpc_subnet.b-subnet-dip.id}"
    ipv4 = true
...
resource "yandex_alb_load_balancer" "nginx-balancer" {
  ...
  allocation_policy {
    location {
      zone_id   = "${yandex_vpc_subnet.d-subnet-dip.zone}"
      subnet_id = yandex_vpc_subnet.d-subnet-dip.id
    }
  }
...
resource "yandex_compute_snapshot_schedule" "snapshot-dip" {
  name = "snapshot-dip"
